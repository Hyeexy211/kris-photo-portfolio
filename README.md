...
- 灯箱标题旁的 `View photo` 可打开 `photo.html?id=照片ID`，每张现有照片都有可复制的独立网址。页面从 Content Service 加载资料；静态托管环境的社交爬虫通常只读取初始 HTML，因此目前的分享卡片仍使用站点通用封面，不能视为逐张照片的社交预览已完成。
- `sitemap.xml` 列出当前 3 个作品集、9 张照片以及首页和旧作品页；新增内容后需要同步更新。`robots.txt` 随项目发布，但 GitHub Pages 的项目站点不能控制域名根路径的 `/robots.txt`，所以搜索引擎是否采用它还要在正式域名上验证。
